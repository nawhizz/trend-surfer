...
전체 활성 보통주(2,556종목), 2020-01-01 ~ 2026-06-20, 초기자본 1억, 추세추종.

### 검증 중 발견한 기존 버그 (별도 수정)

검증 백테스트 실행 중 `ZeroDivisionError`로 중단. 근본 원인은 일부 종목의
결손 캔들(`open/high/low/volume=0`, close만 존재)로 `entry_price=0` 포지션이
생성되어 손익률 계산(`/ entry_price`)에서 발생. **#1 작업과 무관한 기존 결함**으로,
`_process_pending_entries`에 `_is_valid_entry_price` 가드(entry_price<=0이면 진입 취소)를
추가해 별도 커밋(`a6e9a5b`)으로 수정.

> ⚠️ 이 버그 때문에 최초 "변경 전" 기준선(+54.34%)은 **죽기 직전의 오염된 값**이었음.
> 공정한 비교를 위해 "가드 O + 2일청산 OFF(임계값 임시 9999)" 기준선을 별도 측정함.

### 3-way 비교

| 지표 | 가드만 (2일청산 OFF) | **변경 후 (2일청산 ON)** | 효과 |
|------|:---:|:---:|:---:|
| 총 수익률 | +12.41% | **+36.40%** | ✅ 약 3배 |
| CAGR | +1.82% | **+4.91%** | ✅ 2.7배 |
| 총 거래 수 | 176 | 92 | 거래 축소 |
| 승률 | 32.4% | 32.6% | ≈ |
| 손익비 | 1.12 | **1.56** | ✅ |
| 평균 R | +0.33R | **+0.43R** | ✅ |
| MDD | 30.07% | **20.27%** | ✅ 약 10%p 개선 |
| 샤프 | 0.07 | **0.41** | ✅ 약 6배 |
| 평균 보유기간 | 31.2일 | 26.8일 | ✅ 단축 |
| 최대 연속 패 | 11 | **8** | ✅ |

### 결론

2일 연속 EMA 이탈 청산은 **목표를 달성**. 추세 둔화 포지션을 트레일링 스탑까지
끌고 가지 않고 조기 청산함으로써, 부실 손익비(1.12)와 과도한 MDD(30%)를
개선(손익비 1.56, MDD 20.3%, 샤프 0.07→0.41)했다. 수익·위험 지표가 전반적으로 향상되어
롤백 없이 채택. (최초 "수익 감소" 판단은 오염된 기준선에 의한 오해였고, 공정한 측정으로 바로잡음.)
